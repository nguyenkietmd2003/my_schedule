...
const AB = () => {
  ...
  const booking1 = [
    {
      id: 1,
      start_time: "2024-12-05T18:00:00.000Z",
      end_time: "2024-12-05T18:30:00.000Z",
    },
  ];
  const schedule = [
    {
  ...
  const generateSlotsForDay = (date) => {
    ...
    // Gộp tất cả các booking để kiểm tra
    const allBookings = booking1.map((b) => ({
    ...
    finalSchedules.forEach((schedule) => {
      ...
      while (start < end) {
        ...
        if (slotEnd <= end) {
          // Kiểm tra xem slot có bị trùng với booking không
